...
const schedule = ref<FacebookSchedule | null>(null);
const validation = ref<FacebookValidation | null>(null);
...
function errorText(cause: unknown) {
  return (
    ...
    (cause instanceof Error ? cause.message : "L’action Facebook a échoué.")
  );
}

const scheduleLabels: Record<FacebookSchedule["status"], string> = {
  queued: "En attente",
...
const attemptLabels: Record<
  FacebookSchedule["attempts"][number]["result"],
  string
> = {
  started: "En cours",
  success: "Réussie",
...
async function validate() {
  ...
  try {
    ...
    announcement.value = validation.value.valid
      ? "La publication est compatible avec Facebook."
      : "La validation Facebook contient des erreurs.";
  } catch (cause) {
...
async function program() {
  ...
  try {
    ...
    announcement.value = "La publication Facebook est programmée.";
    emit("scheduled");
  } catch (cause) {
    error.value = errorText(cause);
  } finally {
...
async function retry() {
  ...
  try {
    ...
    announcement.value = "La relance Facebook est programmée.";
  } catch (cause) {
    error.value = errorText(cause);
  } finally {
    isLoading.value = false;
...
<template>
  <section
    ...
    aria-labelledby="facebook-publishing-title"
  >
    <h2 id="facebook-publishing-title">Publication Facebook</h2>
    ...
    <p v-else>Aucune programmation Facebook.</p>

    <div v-if="canManage && !schedule" class="facebook-controls">
      <p v-if="!accounts.length">
        Aucun compte Facebook connecté.
        <NuxtLink to="/settings/facebook">Connecter une Page Facebook</NuxtLink
        >.
      </p>
      <template v-else>
        <label for="facebook-account">Page Facebook</label>
        <select id="facebook-account" v-model="accountId">
          <option
        ...
        <label for="facebook-run-at">Date et heure de publication</label>
        <input id="facebook-run-at" v-model="runAt" type="datetime-local" />
    ...
    <button
      ...
      @click="retry"
    >
      Relancer la publication Facebook
    </button>

    <div v-if="schedule?.attempts.length" class="table-scroll" tabindex="0">
      <table>
        <caption>
          Historique des tentatives Facebook
        </caption>
        <thead>
          <tr>
