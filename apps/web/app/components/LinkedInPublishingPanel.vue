...
function errorText(cause: unknown) {
  return (
    ...
    (cause instanceof Error ? cause.message : "L’action LinkedIn a échoué.")
  );
}

const scheduleLabels: Record<LinkedInSchedule["status"], string> = {
  queued: "En attente",
...
const attemptLabels: Record<
  LinkedInSchedule["attempts"][number]["result"],
  string
> = {
  started: "En cours",
  success: "Réussie",
...
async function validate() {
  ...
  try {
    ...
    announcement.value = validation.value.valid
      ? "La publication est compatible avec LinkedIn."
      : "La validation LinkedIn contient des erreurs.";
  } catch (cause) {
...
async function program() {
  ...
  try {
    ...
    announcement.value = "La publication LinkedIn est programmée.";
    emit("scheduled");
  } catch (cause) {
    error.value = errorText(cause);
  } finally {
...
async function retry() {
  ...
  try {
    ...
    announcement.value = "La relance LinkedIn est programmée.";
  } catch (cause) {
    error.value = errorText(cause);
  } finally {
    isLoading.value = false;
...
<template>
  <section
    ...
    aria-labelledby="linkedin-publishing-title"
  >
    <h2 id="linkedin-publishing-title">Publication LinkedIn</h2>
    ...
    <div v-if="canManage && !schedule" class="linkedin-controls">
      <p v-if="!accounts.length">
        ...
        <NuxtLink to="/settings/linkedin">Connecter un compte LinkedIn</NuxtLink
        >.
      </p>
      <template v-else>
        ...
        <input id="linkedin-run-at" v-model="runAt" type="datetime-local" />
    ...
    <button
      ...
      @click="retry"
    >
      Relancer la publication LinkedIn
    </button>

    <div v-if="schedule?.attempts.length" class="table-scroll" tabindex="0">
      <table>
        <caption>
          Historique des tentatives LinkedIn
        </caption>
        <thead>
          <tr>
