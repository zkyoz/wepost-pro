...
export type FacebookProcessorDependencies = {
...
export async function processFacebookPublication(
  job: FacebookJobContract,
  dependencies: FacebookProcessorDependencies,
) {
  ...
  try {
    if (
      ...
      facebookPayloadHash({ text: record.text, media: record.media }) !==
        record.payloadHash
    ) {
      throw {
