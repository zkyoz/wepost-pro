...
export type LinkedInProcessorDependencies = {
...
export async function processLinkedInPublication(
  job: LinkedInJobContract,
  dependencies: LinkedInProcessorDependencies,
) {
  ...
  try {
    if (
      ...
      linkedinPayloadHash({ text: record.text, media: record.media }) !==
        record.payloadHash
    ) {
      throw {
