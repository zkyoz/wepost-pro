...
import {
  toScheduleView,
  toSocialAccountView,
  pinterestMediaForPublication,
  validatePublicationForPinterest,
} from '#services/social/pinterest_service'
import { getPinterestOAuthClient } from '#services/social/pinterest_oauth_client'
import { enqueuePinterestPublication } from '#services/social/pinterest_queue'
...
import {
  pinterestAccountValidator,
  pinterestOAuthCallbackValidator,
  pinterestOAuthStartValidator,
  pinterestPublicationValidator,
  pinterestScheduleValidator,
  pinterestStatusValidator,
} from '#validators/pinterest_validator'
...
const accountNotFound = { errors: [{ message: 'Compte Pinterest introuvable.' }] }
const scheduleNotFound = { errors: [{ message: 'Programmation Pinterest introuvable.' }] }
...
export default class PinterestController {
  async oauthStart({ auth, request, response, session }: HttpContext) {
    ...
    const payload = await request.validateUsing(pinterestOAuthStartValidator)
    ...
    session.put('pinterest_oauth_nonce', nonce)
    const state = encryption.encrypt(JSON.stringify(statePayload), 600, 'pinterest:oauth-state')
    return response.ok({
      data: { authorizationUrl: getPinterestOAuthClient().authorizationUrl(state) },
    })
  }

  async oauthCallback({ auth, logger, request, response, session }: HttpContext) {
    const actor = auth.getUserOrFail()
    const payload = await request.validateUsing(pinterestOAuthCallbackValidator)
    const decrypted = encryption.decrypt(payload.state, 'pinterest:oauth-state')
    const nonce = session.get('pinterest_oauth_nonce')
    session.forget('pinterest_oauth_nonce')
    ...
    try {
      const client = getPinterestOAuthClient()
      const token = await client.exchangeCode(payload.code)
      const [profile, boards] = await Promise.all([
        client.currentUser(token.accessToken),
        client.boards(token.accessToken),
      ])
      const missingScopes = pinterestConfig.scopes.filter((scope) => !token.scopes.includes(scope))
      if (missingScopes.length) {
        return response.unprocessableEntity({
          errors: [{ message: 'Les permissions Pinterest minimales n’ont pas été accordées.' }],
          meta: { missingScopes },
        })
      }
      const account = await db.transaction(async (trx) => {
        const existing = await SocialAccount.query({ client: trx })
          ...
          .where('network', 'pinterest')
          .where('externalAccountId', profile.id)
          .first()
        ...
        await AuditLog.create(
          {
            ...
            action: 'social.pinterest_connected',
            previousValues: existing ? { status: existing.status } : {},
            nextValues: {
              accountId: next.id,
              pinterestUserId: profile.id,
      ...
      logger.info({
        event: 'social.pinterest_connected',
      ...
      return response.badRequest({ errors: [{ message: 'La connexion Pinterest a échoué.' }] })
    }
  }

  async accounts({ auth, response }: HttpContext) {
    const actor = auth.getUserOrFail()
    const query = SocialAccount.query().where('network', 'pinterest').orderBy('createdAt', 'desc')
  ...
  async refresh({ auth, logger, request, response }: HttpContext) {
    const actor = auth.getUserOrFail()
    const { params } = await request.validateUsing(pinterestAccountValidator)
    const query = SocialAccount.query().where('id', params.id).where('network', 'pinterest')
    if (actor.role === 'agency') query.where('agencyId', actor.agencyId!)
    ...
    if (!account.encryptedRefreshToken) {
      return response.unprocessableEntity({
        errors: [
          {
            message:
              'Le renouvellement programmatique est indisponible pour ce compte. Reconnectez Pinterest.',
          },
        ],
      })
    }
    ...
    try {
      const token = await getPinterestOAuthClient().refreshToken(
        ...
        await AuditLog.create(
          {
            ...
            targetProjectId: null,
            targetPublicationId: null,
            action: 'social.pinterest_refreshed',
      ...
      logger.info({ event: 'social.pinterest_refreshed', accountId: account.id, actorId: actor.id })
      return response.ok({ data: toSocialAccountView(account) })
    } catch (error) {
      logger.warn({
        event: 'social.pinterest_refresh_failed',
        accountId: account.id,
        errorName: error instanceof Error ? error.name : 'UnknownError',
      })
      return response.badGateway({
        errors: [{ message: 'Le renouvellement Pinterest a échoué. Reconnectez le compte.' }],
      })
    }
  }
  ...
  async revoke({ auth, logger, request, response }: HttpContext) {
      ...
      await AuditLog.create(
        {
          ...
          targetProjectId: null,
          targetPublicationId: null,
          action: 'social.pinterest_revoked',
    ...
    logger.info({ event: 'social.pinterest_revoked', accountId: account.id, actorId: actor.id })
    return response.ok({ data: toSocialAccountView(account) })
  }

  async validate({ auth, request, response }: HttpContext) {
    const actor = auth.getUserOrFail()
    const payload = await request.validateUsing(pinterestPublicationValidator)
    ...
    if (!account) return response.notFound(accountNotFound)
    const effective = await effectiveNetworkText(publication, 'pinterest')
    const requestedPin = pinFrom(payload)
    const pin =
      effective.source === 'variant'
        ? { ...requestedPin, description: effective.text }
        : requestedPin
    return response.ok({ data: await validatePublicationForPinterest(publication, account, pin) })
  }

  async schedule({ auth, logger, request, response }: HttpContext) {
    const actor = auth.getUserOrFail()
    const payload = await request.validateUsing(pinterestScheduleValidator)
    const publication = await findAccessiblePublication(actor, payload.params.id)
    if (!publication) return response.notFound(publicationNotFound)
    const account = await accountForPublication(payload.accountId, publication.agencyId)
    if (!account) return response.notFound(accountNotFound)
    const idempotencyKey = pinterestIdempotencyKey({
    ...
    const pin =
      effective.source === 'variant'
        ? { ...requestedPin, description: effective.text }
        : requestedPin
    const validation = await validatePublicationForPinterest(publication, account, pin)
    ...
    try {
      schedule = await db.transaction(async (trx) => {
        const media = await pinterestMediaForPublication(publication.id)
        const created = await ScheduledPublication.create(
          {
            ...
            payloadHash: pinterestPayloadHash({
              pin,
              media,
            }),
          },
        ...
        await AuditLog.create(
          {
            ...
            action: 'social.pinterest_scheduled',
    ...
    try {
      await enqueuePinterestPublication({
        scheduledPublicationId: schedule.id,
        jobId: `pinterest-${idempotencyKey}`,
      ...
      logger.error({
        event: 'social.pinterest_enqueue_failed',
        scheduledPublicationId: schedule.id,
        errorName: error instanceof Error ? error.name : 'UnknownError',
      })
    ...
    logger.info({
      event: 'social.pinterest_scheduled',
      scheduledPublicationId: schedule.id,
      actorId: actor.id,
    })
  ...
  async publicationStatus({ auth, request, response }: HttpContext) {
    const actor = auth.getUserOrFail()
    const { params } = await request.validateUsing(pinterestStatusValidator)
  ...
  async retry({ auth, logger, request, response }: HttpContext) {
    ...
    if (!account) return response.notFound(accountNotFound)
    const pin = schedule.networkPayloadJson as PinterestPinPayload
    const validation = await validatePublicationForPinterest(publication, account, pin)
    ...
    try {
      await enqueuePinterestPublication({
        scheduledPublicationId: schedule.id,
        jobId: `pinterest-${schedule.idempotencyKey}-manual-${generation}`,
        runAt: new Date(),
      })
    } catch (error) {
      await markSocialScheduleFailed(schedule.id, publication.id)
      schedule.status = 'failed'
      logger.error({
        event: 'social.pinterest_retry_enqueue_failed',
        scheduledPublicationId: schedule.id,
        errorName: error instanceof Error ? error.name : 'UnknownError',
      })
      return response.serviceUnavailable({
