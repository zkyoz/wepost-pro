...
import { instagramIdempotencyKey, instagramPayloadHash } from '#domain/social/instagram'
...
import {
  toScheduleView,
  toSocialAccountView,
  instagramMediaForPublication,
  validatePublicationForInstagram,
} from '#services/social/instagram_service'
import { getInstagramOAuthClient } from '#services/social/instagram_oauth_client'
import { enqueueInstagramPublication } from '#services/social/instagram_queue'
...
import {
  ...
  instagramOAuthCallbackValidator,
  instagramOAuthStartValidator,
  instagramPublicationValidator,
  instagramScheduleValidator,
  instagramStatusValidator,
} from '#validators/instagram_validator'
...
const accountNotFound = { errors: [{ message: 'Compte Instagram introuvable.' }] }
const scheduleNotFound = { errors: [{ message: 'Programmation Instagram introuvable.' }] }
...
export default class InstagramController {
  ...
  async oauthCallback({ auth, logger, request, response, session }: HttpContext) {
    const actor = auth.getUserOrFail()
    const payload = await request.validateUsing(instagramOAuthCallbackValidator)
    const decrypted = encryption.decrypt(payload.state, 'instagram:oauth-state')
    const nonce = session.get('instagram_oauth_nonce')
    session.forget('instagram_oauth_nonce')
    ...
    try {
      const client = getInstagramOAuthClient()
      ...
      const page = pages.find((candidate) => candidate.id === state.instagramAccountId)
      if (!page) {
        return response.unprocessableEntity({
          errors: [{ message: 'Le compte Instagram professionnel sélectionné est introuvable.' }],
        })
      }
      const missingScopes = instagramConfig.scopes.filter((scope) => !grantedScopes.includes(scope))
      if (missingScopes.length) {
        return response.unprocessableEntity({
          errors: [{ message: 'Les permissions Instagram minimales n’ont pas été accordées.' }],
          meta: { missingScopes },
        })
      }
        ...
        await AuditLog.create(
          {
            ...
            nextValues: { accountId: next.id, instagramAccountId: page.id, status: 'connected' },
          },
          { client: trx }
        )
        return next
      ...
      logger.info({
        event: 'social.instagram_connected',
      ...
      return response.badRequest({ errors: [{ message: 'La connexion Instagram a échoué.' }] })
    }
  }
  ...
  async revoke({ auth, logger, request, response }: HttpContext) {
    const actor = auth.getUserOrFail()
    const { params } = await request.validateUsing(instagramAccountValidator)
    const query = SocialAccount.query().where('id', params.id).where('network', 'instagram')
    if (actor.role === 'agency') query.where('agencyId', actor.agencyId!)
      ...
      await AuditLog.create(
        {
          ...
          targetProjectId: null,
          targetPublicationId: null,
          action: 'social.instagram_revoked',
    ...
    logger.info({ event: 'social.instagram_revoked', accountId: account.id, actorId: actor.id })
    return response.ok({ data: toSocialAccountView(account) })
  }

  async validate({ auth, request, response }: HttpContext) {
    const actor = auth.getUserOrFail()
    const payload = await request.validateUsing(instagramPublicationValidator)
    ...
    if (!account) return response.notFound(accountNotFound)
    const effective = await effectiveNetworkText(publication, 'instagram')
    return response.ok({
      data: await validatePublicationForInstagram(publication, account, effective.text),
    })
  }

  async schedule({ auth, logger, request, response }: HttpContext) {
    const actor = auth.getUserOrFail()
    const payload = await request.validateUsing(instagramScheduleValidator)
    const publication = await findAccessiblePublication(actor, payload.params.id)
    if (!publication) return response.notFound(publicationNotFound)
    const account = await accountForPublication(payload.accountId, publication.agencyId)
    if (!account) return response.notFound(accountNotFound)
    const idempotencyKey = instagramIdempotencyKey({
    ...
    const effective = await effectiveNetworkText(publication, 'instagram')
    const validation = await validatePublicationForInstagram(publication, account, effective.text)
    ...
    try {
      schedule = await db.transaction(async (trx) => {
        const media = await instagramMediaForPublication(publication.id)
        const created = await ScheduledPublication.create(
          {
            ...
            payloadHash: instagramPayloadHash({
              text: effective.text,
              media,
            }),
          },
        ...
        await AuditLog.create(
          {
            ...
            action: 'social.instagram_scheduled',
    ...
    try {
      await enqueueInstagramPublication({
        scheduledPublicationId: schedule.id,
        jobId: `instagram-${idempotencyKey}`,
      ...
      logger.error({
        event: 'social.instagram_enqueue_failed',
        scheduledPublicationId: schedule.id,
        errorName: error instanceof Error ? error.name : 'UnknownError',
      })
    ...
    logger.info({
      event: 'social.instagram_scheduled',
      scheduledPublicationId: schedule.id,
      actorId: actor.id,
    })
  ...
  async publicationStatus({ auth, request, response }: HttpContext) {
    const actor = auth.getUserOrFail()
    const { params } = await request.validateUsing(instagramStatusValidator)
  ...
  async retry({ auth, logger, request, response }: HttpContext) {
    ...
    const validation = await validatePublicationForInstagram(publication, account, frozenText)
    ...
    try {
      await enqueueInstagramPublication({
        scheduledPublicationId: schedule.id,
        jobId: `instagram-${schedule.idempotencyKey}-manual-${generation}`,
        runAt: new Date(),
      })
    } catch (error) {
      await markSocialScheduleFailed(schedule.id, publication.id)
      schedule.status = 'failed'
      logger.error({
        event: 'social.instagram_retry_enqueue_failed',
        scheduledPublicationId: schedule.id,
        errorName: error instanceof Error ? error.name : 'UnknownError',
      })
      return response.serviceUnavailable({
