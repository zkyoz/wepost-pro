import { validateFacebookPublication } from '#domain/social/facebook'
...
export async function facebookMediaForPublication(publicationId: string) {
  const rows = await db
...
export async function validatePublicationForFacebook(
  publication: Publication,
  account: SocialAccount,
  effectiveText: string = publication.baseText
) {
  return validateFacebookPublication({
    ...
    media: await facebookMediaForPublication(publication.id),
  })
}
