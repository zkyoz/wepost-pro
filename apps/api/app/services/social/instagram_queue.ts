...
export const INSTAGRAM_JOB_NAME = 'publication-instagram'
export type InstagramPublicationJob = { scheduledPublicationId: string }

let queue: Queue<InstagramPublicationJob> | undefined

function getQueue() {
  queue ??= new Queue<InstagramPublicationJob>(env.get('EMAIL_QUEUE_NAME'), {
    connection: {
...
export async function enqueueInstagramPublication(input: {
