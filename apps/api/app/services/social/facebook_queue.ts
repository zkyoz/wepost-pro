...
export const FACEBOOK_JOB_NAME = 'publication-facebook'
export type FacebookPublicationJob = { scheduledPublicationId: string }

let queue: Queue<FacebookPublicationJob> | undefined

function getQueue() {
  queue ??= new Queue<FacebookPublicationJob>(env.get('EMAIL_QUEUE_NAME'), {
    connection: {
...
export async function enqueueFacebookPublication(input: {
