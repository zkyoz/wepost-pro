...
export const PINTEREST_JOB_NAME = 'publication-pinterest'
export type PinterestPublicationJob = { scheduledPublicationId: string }

let queue: Queue<PinterestPublicationJob> | undefined

function getQueue() {
  queue ??= new Queue<PinterestPublicationJob>(env.get('EMAIL_QUEUE_NAME'), {
    connection: {
...
export async function enqueuePinterestPublication(input: {
